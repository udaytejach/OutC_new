...
class VisaReportsPage extends StatefulWidget {
  const VisaReportsPage({super.key});

  @override
  State<VisaReportsPage> createState() => _VisaReportsPageState();
}

class _VisaReportsPageState extends State<VisaReportsPage> {
  ...
  Widget build(BuildContext context) {
    var reportsProvider = context.watch<VisaReportsProvider>();

    reportsProvider.loadVisaReportsList();
    ...
    return SizedBox(
      child: Scaffold(
        ...
        body: Column(
          children: [
            getheadercontainer(),
            Selector<VisaReportsProvider, ApiResponse<GetVisaReportsResponse>?>(
              selector: (_, apiResponse) => apiResponse.getVisaReportsResponse,
              builder: (_, instance, __) {
                if (instance?.status == Status.error) {
                  ...
                  List<Datum?> visaReportsListData = instance?.data?.data ?? [];
                  return visaReportsListData.isEmpty
                      ? Column(
                          children: [
                            const SizedBox(
                              height: 50,
                      ...
                      : Expanded(
                          child: ListView.builder(
                              shrinkWrap: true,
                              itemCount: visaReportsListData.length,
                                                ...
                                                children: <Widget>[
                                                  ...
                                                  Row(
                                                    children: [
                                                      ...
                                                      Text(
                                                        "${visaReportsListData[index]?.customerFirstName!} ${visaReportsListData[index]!.customerLastName}" ??
                                                            "",
                                                        style: const TextStyle(
                                                          fontSize: 14.0,
                                                          color: Colors.black,
