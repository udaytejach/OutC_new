...
class FlightrepPage extends StatefulWidget {
  const FlightrepPage({super.key});

  @override
  State<FlightrepPage> createState() => _FlightrepPageState();
}

class _FlightrepPageState extends State<FlightrepPage> {
  ...
  Widget build(BuildContext context) {
    var reportsProvider = context.watch<FlightReportsProvider>();

    reportsProvider.loadFlightReportsList();
    ...
    return SizedBox(
      child: Scaffold(
        ...
        body: Column(
          children: [
            getheadercontainer(),
            Selector<FlightReportsProvider,
                ApiResponse<GetFlightReportsResponse>?>(
              ...
              builder: (_, instance, __) {
                if (instance?.status == Status.error) {
                  ...
                  List<Datum?> flightReportsListData =
                      instance?.data?.data ?? [];
                  return flightReportsListData.isEmpty
                      ? Column(
                          children: [
                            const SizedBox(
                              height: 50,
                      ...
                      : Expanded(
                          child: ListView.builder(
                              shrinkWrap: true,
                              itemCount: flightReportsListData.length,
                                                    ...
                                                    children: [
                                                      ...
                                                      Text(
                                                        flightReportsListData[
                                                            ...
                                                            : DateFormat(
                                                                    'dd-MM-yyyy')
                                                                .format(DateTime.parse(flightReportsListData[
                                                                            index]
                                                                        ?.oneWaySegment?[
                                                                            0]
                                                                        .arrivalDateTime
