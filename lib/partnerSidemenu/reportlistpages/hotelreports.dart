...
class HotelReportsPage extends StatefulWidget {
  const HotelReportsPage({super.key});

  @override
  State<HotelReportsPage> createState() => _HotelReportsPageState();
}

class _HotelReportsPageState extends State<HotelReportsPage> {
  ...
  Widget build(BuildContext context) {
    var reportsProvider = context.watch<ReportsProvider>();

    reportsProvider.loadHotelReportsList();
    ...
    return SizedBox(
      child: Scaffold(
        ...
        body: Column(
          children: [
            getheadercontainer(),
            Selector<ReportsProvider, ApiResponse<GetHotelReportsResponse>?>(
              selector: (_, apiResponse) => apiResponse.getHotelReportsResponse,
              builder: (_, instance, __) {
                if (instance?.status == Status.error) {
                  ...
                  List<Datum?> hotelReportsListData =
                      instance?.data?.data ?? [];
                  return hotelReportsListData.isEmpty
                      ? Column(
                          children: [
                            const SizedBox(
                              height: 50,
                      ...
                      : Expanded(
                          child: ListView.builder(
                              shrinkWrap: true,
                              itemCount: hotelReportsListData.length,
                                                    ...
                                                    children: [
                                                      ...
                                                      Text(
                                                        hotelReportsListData[
                                                            ...
                                                            : DateFormat(
                                                                    'dd-MM-yyyy')
                                                                .format(DateTime.parse(hotelReportsListData[
                                                                            index]
                                                                        ?.checkInDate
                                                                        .toString() ??
                                                                    "")),
